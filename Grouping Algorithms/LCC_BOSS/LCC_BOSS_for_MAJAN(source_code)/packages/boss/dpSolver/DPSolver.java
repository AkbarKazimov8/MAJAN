    ...
    {    	
         ...
         for (int curSize = 2; curSize <= numOfAgents; curSize++)
        {
         	...
         	if( curSize < numOfAgents )
        	{
         	...
         	if( curSize < numOfAgents ){
            	//System.out.println("UpdateIPSolution: runDPorIDP11");
   				...
   				result.updateSolutionList(bestCSFoundSoFar_byteFormat, +input.getCoalitionStructureValue( bestCSFoundSoFar_byteFormat ));
   				if(( input.solverName == SolverNames.ODPIP )||( input.solverName == SolverNames.ODPinParallelWithIP )) {
    	...
    	//System.out.println("UpdateIPSolution: runDPorIDP222");

        result.updateDPSolution( dpBestCSInByteFormat , input.getCoalitionStructureValue( dpBestCSInByteFormat ) );
        result.updateSolutionList(dpBestCSInByteFormat, input.getCoalitionStructureValue( dpBestCSInByteFormat ) );
        if(( input.solverName == SolverNames.ODPIP )||( input.solverName == SolverNames.ODPinParallelWithIP )) {
