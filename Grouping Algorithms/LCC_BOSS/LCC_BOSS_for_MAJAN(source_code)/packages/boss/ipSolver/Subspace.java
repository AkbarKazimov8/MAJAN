...
import boss.general.RandomPermutation;
import boss.general.RandomSubsetOfGivenSet;
...
public class Subspace
{
	...
	public int search( Input input, Output output, Result result, double acceptableValue, double[] avgValueForEachSize, double[] sumOfMax, int numOfIntegersToSplit)
	{
		...
		if( input.useSamplingWhenSearchingSubspaces ){
			...
			if( input.solverName == SolverNames.ODPIP ){
				...
				result.updateSolutionList(CS, input.getCoalitionStructureValue(CS));
	...
	{ 	    	
 		...
 		for(int i=0; i<numOfSamples; i++)
		{
			...
			result.updateSolutionList( CSInByteFormat, coalitionStructureValue);
	...
	{ 	    	
		...
		{			
			...
			result.updateSolutionList( CSInByteFormat, input.getCoalitionStructureValue(CS));
...
//				int[][] CSInByteFormat = Combinations.convertSetOfCombinationsFromBitToByteFormat(CS,input.numOfAgents,integers);
		...
		main_loop: while(true)
		{
			if(( (ODPIsHelpingIP)||(ODPinParallelWithIP) )&&( result.get_dpHasFinished() )){  
 			...
 			do{ 
				...
				{
 					//System.out.println("------localbrancbound--11-" + localBranchAndBoundObject.getValueOfBestPartitionFound());
 				//	System.out.println("------localbrancbound-22--" + localBranchAndBoundObject.getValueOfBestPartitionFound( CS[numOfIntegers-1]));
					...
					result.updateSolutionList( CSInByteFormat, valueOfCS);
	...
	private void searchFirstOrLastLevel(int[] integers, Input input, Output output, Result result)
	{
		...
		result.updateSolutionList( curCS, valueOfCurCS);
