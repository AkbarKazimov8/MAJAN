...
public class HDBSCANStarRunner {
	...
	public int[] execute(Double[][] similarityScores, ArrayList<Constraint> constraints, int minPts, int minClSize) {
		...
		File hdbscanFolder = new File(s + "/HDBSCANFiles");
		hdbscanFolder.mkdirs();
		String fileName = "/hdbscan_execution";
		parameters.hierarchyFile = hdbscanFolder.getAbsolutePath() + fileName + "_hierarchy.csv";
		parameters.clusterTreeFile = hdbscanFolder.getAbsolutePath() + fileName + "_tree.csv";
		parameters.partitionFile = hdbscanFolder.getAbsolutePath() + fileName + "_partition.csv";
		parameters.outlierScoreFile = hdbscanFolder.getAbsolutePath() + fileName + "_outlier_scores.csv";
		parameters.visualizationFile = hdbscanFolder.getAbsolutePath() + fileName + "_visualization.vis";
		parameters.compactHierarchy = false;

		int numPoints = similarityScores.length;

		// Compute core distances:
		long startTime = System.currentTimeMillis();
		double[] coreDistances = HDBSCANStar.calculateCoreDistances(similarityScores, minPts);

		System.out.println("Time to compute core distances (ms): " + (System.currentTimeMillis() - startTime));

		// Calculate minimum spanning tree:
		startTime = System.currentTimeMillis();
		UndirectedGraph mst = HDBSCANStar.constructMST(similarityScores, coreDistances, true);
		...
		try {
			startTime = System.currentTimeMillis();
			clusters = HDBSCANStar.computeHierarchyAndClusterTree(mst, minClSize, parameters.compactHierarchy,
					constraints, parameters.hierarchyFile, parameters.clusterTreeFile, ",", pointNoiseLevels,
					pointLastClusters, parameters.visualizationFile);
			System.out.println(
					"Time to compute hierarchy and cluster tree (ms): " + (System.currentTimeMillis() - startTime));
		...
		int[] finalClustering = null;
		try {
			startTime = System.currentTimeMillis();
			finalClustering = HDBSCANStar.findProminentClusters(clusters, parameters.hierarchyFile,
					parameters.partitionFile, ",", numPoints, infiniteStability);
		...
		/*
		 * System.out.println("uzunlu: "+pointLastClusters.length); for (int i = 0; i <
		 * pointLastClusters.length; i++) {
		 * System.out.println("i: "+(i+1)+"--:"+pointLastClusters[i]); }
		 * 
		 * System.out.println("uzunlu2: "+finalClustering.length); for (int j = 0; j <
		 * finalClustering.length; j++) {
		 * System.out.println("j: "+(j+1)+"--:"+finalClustering[j]); }
		 */

		// !!! IMPORTANT!!! ASSIGING OUTLIERS TO CLUSTERS:
		// IF REQUIRED, COMMENT TO PRODUCE OUTLIERS.
		// THIS IS IMPLEMENTED SINCE THE ALGORITHM PRODUCES LOTS OF OUTLIERS AND THE USE
		// CASE
		// REQUIRES MINIMUM OUTLIERS IF POSSIBLE
		if (finalClustering.length == pointLastClusters.length) {
			for (int i = 0; i < finalClustering.length; i++) {
				if (finalClustering[i] == 0) {
					finalClustering[i] = pointLastClusters[i];
				}
			}
		}
		...
		try {
			startTime = System.currentTimeMillis();
			HDBSCANStar.calculateOutlierScores(clusters, pointNoiseLevels, pointLastClusters, coreDistances,
					parameters.outlierScoreFile, ",", infiniteStability);
	 ...
	 * comma-separated value (CSV) file, and that all of the output files will be
	 * CSV files as well. The flags "file=", "minPts=", "minClSize=",
	 * "constraints=", and "distance_function=" should be used to specify the input
	 * data set file, value for minPoints, value for minClusterSize, input
	...
	public static void main(String[] args) {
		...
		System.out.println(
				"Running HDBSCAN* on " + parameters.inputFile + " with minPts=" + parameters.minPoints + ", minClSize="
						+ parameters.minClusterSize + ", constraints=" + parameters.constraintsFile + ", compact="
						+ parameters.compactHierarchy + ", dist_function=" + parameters.distanceFunction.getName());
		...
		// Compute core distances:
		long startTime = System.currentTimeMillis();
		double[] coreDistances = HDBSCANStar.calculateCoreDistances(dataSet, parameters.minPoints,
		...
		try {
			...
			clusters = HDBSCANStar.computeHierarchyAndClusterTree(mst, parameters.minClusterSize,
					parameters.compactHierarchy, constraints, parameters.hierarchyFile, parameters.clusterTreeFile, ",",
					pointNoiseLevels, pointLastClusters, parameters.visualizationFile);
			System.out.println(
					"Time to compute hierarchy and cluster tree (ms): " + (System.currentTimeMillis() - startTime));
		} catch (IOException ioe) {
			System.err.println("Error writing to hierarchy file or cluster tree file.");
	...
	/**
	 * Parses out the input parameters from the program arguments. Prints out a help
	 * message and exits the program if the parameters are incorrect.
	 * 
