...
public class HDBSCANStar {
	...
	public static double[][] readInDataSet(String fileName, String delimiter) throws IOException {
		...
		/*double[][] myfinalDataSet = new double[5][2];
		for(int i =0; i<5;i++) {
			finalDataSet[i][0]=1;
			finalDataSet[i][1]=1;
		}*/
	...
	public static double[] calculateCoreDistances(double[][] dataSet, int k, DistanceCalculator distanceFunction) {
		...
		for (int point = 0; point < dataSet.length; point++) {
			...
			for (int neighbor = 0; neighbor < dataSet.length; neighbor++) {
				...
				//double distance = distanceFunction.getDistance(point, neighbor);
				System.out.println("point:"+point+ " neighbor:"+neighbor+" dist:"+distance);
//TODO return the distance from precomputed input
	...
	public static double[] calculateCoreDistances(Double[][] similarityValues, int k) {
		int numNeighbors = k -1;
		double[] coreDistances = new double[similarityValues.length];

		if (k == 1) {
			for (int point = 0; point < similarityValues.length; point++) {
				coreDistances[point] = 0;
			}
			return coreDistances;
		}
		...
		for (int point = 0; point < similarityValues.length; point++) {
			...
			for (int neighbor = 0; neighbor < similarityValues.length; neighbor++) {
				if (point == neighbor)
					continue;
				//double distance = distanceFunction.computeDistance(dataSet[point], dataSet[neighbor]);
				double distance = similarityValues[point][neighbor];
				//System.out.println("point:"+point + " neighbor:"+neighbor);

//				System.out.println("-----distance1---"); 
	//			System.out.println((point+1)+" vs "+(neighbor+1)+": " + distance);
				//double distance = distanceFunction.getDistance(point, neighbor);
	...
	public static UndirectedGraph constructMST(double[][] dataSet, double[] coreDistances, 
		...
		while (numAttachedPoints < dataSet.length) {
			...
			for (int neighbor = 0; neighbor < dataSet.length; neighbor++) {
				...
				//double distance = distanceFunction.getDistance(currentPoint, neighbor);
				System.out.println("currentPoint:"+currentPoint+ " neighbor:"+neighbor+" dist:"+distance);
	...
	public static UndirectedGraph constructMST(Double[][] similarityValues, double[] coreDistances, 
			boolean selfEdges) {

		int selfEdgeCapacity = 0;
		if (selfEdges)
			selfEdgeCapacity = similarityValues.length;

		//One bit is set (true) for each attached point, or unset (false) for unattached points:
		BitSet attachedPoints = new BitSet(similarityValues.length);

		//Each point has a current neighbor point in the tree, and a current nearest distance:
		int[] nearestMRDNeighbors = new int[similarityValues.length-1 + selfEdgeCapacity];
		double[] nearestMRDDistances = new double[similarityValues.length-1 + selfEdgeCapacity];
		...
		//The MST is expanded starting with the last point in the data set:
		int currentPoint = similarityValues.length-1;
		int numAttachedPoints = 1;
		attachedPoints.set(similarityValues.length-1);

		//Continue attaching points to the MST until all points are attached:
		while (numAttachedPoints < similarityValues.length) {
			int nearestMRDPoint = -1;
			double nearestMRDDistance = Double.MAX_VALUE;
			...
			for (int neighbor = 0; neighbor < similarityValues.length; neighbor++) {
				...
				double distance = similarityValues[currentPoint][neighbor];
			//	System.out.println("currentPoint:"+currentPoint + " neighbor:"+neighbor);

				//System.out.println("-----distance2---"); 
			//	System.out.println((currentPoint+1)+" vs "+(neighbor+1)+": " + distance);
//				double distance = distanceFunction.computeDistance(dataSet[currentPoint], dataSet[neighbor]);
				//double distance = distanceFunction.getDistance(currentPoint, neighbor);
		...
		//Create an array for vertices in the tree that each point attached to:
		int[] otherVertexIndices = new int[similarityValues.length-1 + selfEdgeCapacity];
		for (int i = 0; i < similarityValues.length-1; i++) {
			otherVertexIndices[i] = i;
		}
		...
		//If necessary, attach self edges:
		if (selfEdges) {
			for (int i = similarityValues.length-1; i < similarityValues.length*2-1; i++) {
				int vertex = i - (similarityValues.length-1);
				nearestMRDNeighbors[i] = vertex;
				otherVertexIndices[i] = vertex;
				nearestMRDDistances[i] = coreDistances[vertex];
			}
		...
		return new UndirectedGraph(similarityValues.length, nearestMRDNeighbors, otherVertexIndices, nearestMRDDistances);
	}
